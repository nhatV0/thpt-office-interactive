...
def solve_kth_largest(arr: list, k: int) -> int:
...
def gen_test_data(cmd, n_val, seed):
    random.seed(seed)
    MAX_VAL = 10**9  # Phép nâng lũy thừa chính xác trong Python (10^9)
...
def build_testcases():
    ...
    for idx, (cmd, n_val, seed) in enumerate(commands, 1):
        ...
        # Ghi file input (.inp)
        inp_file = os.path.join(test_dir, "kthlargest.inp")
        with open(inp_file, "w", encoding="utf-8") as f:
            f.write(f"{n} {k}\n")
            f.write(" ".join(map(str, arr)) + "\n")

        # Tính và ghi file output (.out)
        ans = solve_kth_largest(arr, k)
        out_file = os.path.join(test_dir, "kthlargest.out")
        with open(out_file, "w", encoding="utf-8") as f:
            f.write(f"{ans}\n")

    # Nén file ZIP
    zip_path = "So_Lon_Thu_K_Testcases.zip"

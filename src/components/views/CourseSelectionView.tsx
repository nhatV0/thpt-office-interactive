...
export const CourseSelectionView: React.FC<CourseSelectionViewProps> = ({
  ...
  const groupedSections = useMemo(() => {
    ...
    return COURSE_GROUPS.map(group => {
      ...
      if (!showLockedCourses && group.courses.length === 0) {
        return false;
      }
      return true;
    });
  }, [searchQuery, activeFilter, showLockedCourses, isCourseUnlocked]);
        ...
        {groupedSections.map(group => {
          ...
          return (
            <section key={group.id} className="space-y-4">
              ...
              {/* Courses Grid inside this group: Minimalist UI, title and progress only, details on hover */}
              {hasCourses ? (
                  ...
                  {group.courses.map(course => {
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-2xl border transition-all duration-200 flex flex-col justify-between overflow-visible relative group ${
                          isUnlocked
                            ? 'bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800/90 shadow-xs hover:shadow-md hover:border-sky-500/50 dark:hover:border-sky-500/50 hover:-translate-y-0.5 cursor-pointer'
                            : 'bg-slate-50/60 dark:bg-slate-950/40 border-slate-200/60 dark:border-slate-800/60 opacity-60 cursor-not-allowed'
                        }`}
                      >
                        {/* Accent Bar */}
                        <div className={`h-1.5 w-full bg-gradient-to-r ${course.accentClass} rounded-t-2xl`} />

                        <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                          {/* Top: Icon + Title + Hover Tooltip */}
                          <div className="flex items-start gap-3">
                            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${course.accentClass} flex items-center justify-center shadow-xs shrink-0 text-white`}>
                            ...
                            <div className="min-w-0 flex-1 relative">
                              <div className="flex items-center justify-between gap-1">
                                ...
                                {!isUnlocked && (
                                  <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-rose-500 px-1.5 py-0.5 rounded bg-rose-50 dark:bg-rose-950/60">
                              ...
                              {/* Course Title */}
                              <h3 className="text-sm font-bold text-slate-900 dark:text-white truncate group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors mt-0.5">
                                {course.title}
                              </h3>

                              {/* Rich Tooltip on Hover */}
                              <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-40 w-64 p-3.5 rounded-2xl bg-slate-900 dark:bg-slate-800 text-white text-xs shadow-2xl border border-slate-700 pointer-events-none animate-fadeIn">
                                <div className="font-bold text-sky-400 mb-1 text-xs">{course.title}</div>
                                <p className="text-[11px] font-medium text-slate-200 mb-1.5">{course.subtitle}</p>
                                <p className="text-[11px] text-slate-400 leading-relaxed line-clamp-3">{course.description}</p>
                                <div className="mt-2.5 pt-2 border-t border-slate-700/80 flex items-center justify-between text-[10px] text-slate-300 font-semibold">
                          ...
                          {isUnlocked ? (
                            ...
                            <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 text-[11px] text-slate-400 italic">
                        ...
                        {/* Bottom Bar: Action */}
                        {isUnlocked && (
                          <div className="px-4 py-2 bg-slate-50/50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs rounded-b-2xl">
                            <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">
                              {progress.completed > 0 ? 'Đang học dở' : 'Chưa bắt đầu'}
                            </span>
                            <span className="inline-flex items-center gap-1 font-bold text-sky-600 dark:text-sky-400 group-hover:translate-x-0.5 transition-transform text-[11px]">
                              <span>{progress.completed > 0 ? 'Học tiếp' : 'Vào học'}</span>
